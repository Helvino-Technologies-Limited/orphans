...
exports.create = async (req, res) => {
  try {
    ...
    const result = await db.query(
      ...
      [child_id, record_type, description, diagnosis, treatment, doctor_name, visit_date, next_visit, medications, req.user.id]
    );
...
exports.addVaccination = async (req, res) => {
  try {
    ...
    const result = await db.query(
      ...
      [child_id, vaccine_name, date_given, next_due, given_by, notes]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ message: 'Server error.' });
